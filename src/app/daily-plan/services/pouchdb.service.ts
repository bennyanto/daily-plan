...
export class PouchdbService implements PlanQL {
  ...
  fetchPlans(): Promise<Plan[]> { // recupera tutti i piani giornaliri e vecchi
    return this.localDB
  ...
  deletePlan(plan: Plan): Promise<any> { // cancella dal db
      return this.localDB.remove(plan);
  }

  private updatePlan(plan: Plan): Promise<any>  { // aggiorna il piano del db
    return this.localDB.put(plan);
  }
  private createPlan(plan: Plan): Promise<any> { // creo nuovo piano nel db
